"""
WellFound (formerly AngelList) job board implementation
"""
import time
import logging
...
class WellFoundBoard(JobBoardBase):
    """WellFound job board implementation"""
    ...
    def login(self):
        ...
        if not self.credentials.get("email") or not self.credentials.get("password"):
            logger.warning("No WellFound credentials found")
            return False
        ...
        try:
            ...
            logger.error(f"Error logging in to WellFound: {e}")
            return False
    ...
    def search_jobs(self, keywords, location):
        ...
        try:
            # Build search URL
            search_url = f"https://wellfound.com/jobs/tech-sales-jobs"
    ...
    def apply_to_job(self, job_data):
        ...
        try:
            ...
            try:
                ...
                logger.warning("Could not find expected form elements on WellFound")
                return False
                
        except Exception as e:
            logger.error(f"Error during WellFound application: {e}")
            return False 